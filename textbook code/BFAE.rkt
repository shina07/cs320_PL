...
(define (interp bfae ds st)
  (type-case BFAE bfae
    ...
    [add (l r)    (type-case Value*Store (interp l ds st)
                    [v*s (v1 st1)
                         (type-case Value*Store (interp r ds st1)
                           [v*s (v2 st2)
                                (v*s (num+ v1 v2) st2)])])]
    ...
    [app (f a)    (type-case Value*Store (interp f ds st)
                    [v*s (f-val st1)
                         (type-case Value*Store (interp a ds st1)
                           [v*s (a-val st2)
    ...
    [newbox (v)   (type-case Value*Store (interp v ds st)
                    [v*s (v1 st1)
                         (local [(define a (malloc st1))]
                           (v*s (boxV a)
                                (aSto a v1 st1)))])]
    [setbox (b v) (type-case Value*Store (interp b ds st)
                    [v*s (b-val st2)
                         (type-case Value*Store (interp v ds st2)
                           [v*s (v2 st3)
                                (v*s v2
...
(run "{with {x {+ 5 5}} {+ x x}}")
(parse "{with {q {newbox 10}}{setbox {seqn {setbox q 12} q}{openbox q}}}")
